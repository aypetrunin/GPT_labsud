...
class gpt_helper:
    ...
    def ClientAI(self):
        if not self.clientOpenAI:
            self.clientOpenAI = AsyncOpenAI(api_key=ixapp.config.OPENAI_API_KEY)
    ...
    async def print_usage_tokens(self, prompt_tokens, completion_tokens):
        d2 = datetime.now()
        sec = (d2 - self.d1).total_seconds()
        text = f"использовано. вопрос: {prompt_tokens}, ответ:{completion_tokens} токенов ({sec}с)"
    ...
    async def completion(self, usercontent, refined_query):
        ...
        await self.print_usage_tokens( self.prompt_tokens, self.completion_tokens)
        return full_response

    async def answer_gpt(self):
        await self.db.new_user()
        h = self
        await h.pause()
        try:
            ...
            if not is_query:
                ...
                if len(h.messages) > 1:
                    h.edit_text("Уточняю вопрос...")
                    refined_query = h.query_refiner(h.query)
                    h.edit_text(f"Отвечаю на вопрос: {refined_query}")
                else:
                ...
                logger.debug(text)
                await self.db.answer_usermessage( h.topic_message.message_id, db_response, self.prompt_tokens, self.completion_tokens)
